...
- Make archive extraction safely reusable: `ExtractorService.extract()` now stages extraction in a temporary directory, serializes concurrent publishers with JVM and cross-process locks, and atomically publishes a completion-marked target. Only completed targets are reused, so an interrupted extraction cannot be mistaken for a valid IDE; the reuse check remains inside the extraction `BuildService` so it is not recorded as a configuration-cache input
