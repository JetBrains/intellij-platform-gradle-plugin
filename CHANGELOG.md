...
- Resolve the `kotlin.stdlib.default.dependency` property the same way as the Kotlin Gradle plugin when running `verifyPluginProjectConfiguration`, honoring a value defined in a subproject's own `gradle.properties` (or provided as an extra project property by the IntelliJ Platform settings plugin) instead of only the root project, fixing a false-positive "Kotlin stdlib dependency conflict" warning JetBrains/intellij-platform-gradle-plugin#1789
