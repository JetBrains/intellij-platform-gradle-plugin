...
- Attach plugin API sources bundled in a plugin distribution's `lib/src` directory. When depending on a plugin that bundles its public API sources (`plugin(...)`/`bundledPlugin(...)`/`localPlugin(...)`), the `lib/src/*.jar` source JARs are now collected and exposed as a dedicated sources variant so the IDE attaches them automatically to the plugin library, restoring the documented `1.x` behavior. The source JARs are surfaced only as sources and never added to the compile or runtime classpath JetBrains/intellij-platform-gradle-plugin#2033
