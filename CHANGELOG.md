...
- Add per-plugin bundled-library exclusions with `plugin(...) { excludeBundledLibrary("*foo-.jar") }`, applied consistently to project classpaths and sandbox installations JetBrains/intellij-platform-gradle-plugin#587
...
- Organize plugin tasks into dedicated Gradle task groups instead of a single `intellij platform` group: keep build/run/publishing tasks (`buildPlugin`, `buildPluginVariants`, `signPlugin`, `publishPlugin`, `runIde`, `testIde`, `testIdeUi`, …) in `intellij platform`, move verification tasks (`verifyPlugin`, `verifyPluginProjectConfiguration`, `verifyPluginSignature`, `verifyPluginStructure`, `testIdePerformance`) to `intellij platform verification`, move informational tasks (`printBundledPlugins`, `printBundledModules`, `printProductsReleases`) to `intellij platform info`, and hide internal/plumbing tasks (e.g. `composedJar`, `instrumentCode`, `generateManifest`, `patchPluginXml`, `prepareSandbox`, `buildSearchableOptions`, and the per-target `buildPluginVariants_<os>_<arch>` tasks) from the task list JetBrains/intellij-platform-gradle-plugin#1886
- Fully honor Gradle's `--offline` mode across the plugin's custom, out-of-band network paths. When the build runs offline, the plugin no longer fetches product releases listings, installer/IDE download URLs, `latest`/`LATEST-EAP-SNAPSHOT` resolution, JetBrains Marketplace listings via the localhost shim, or `maven-metadata.xml`; it reuses already-cached artifacts and fails fast with a clear, actionable message naming the missing artifact instead of hanging or throwing an opaque network error. The offline flag is captured at configuration time from `StartParameter.isOffline` and threaded into each path as a tracked input to keep configuration cache correct.
...
- Run the IntelliJ Plugin Verifier against each verified IDE's own bundled JetBrains Runtime (JBR) instead of the runtime used to build the plugin, so plugin classes are resolved against the correct runtime per IDE. The `verifyPlugin` task no longer passes `-runtime-dir` by default; the resolved runtime is exposed via `JAVA_HOME` and used only as a fallback for IDEs without a bundled JBR. Set `intellijPlatform { pluginVerification { useBundledRuntime = false } }` (or the `verifyPlugin` task's `useBundledRuntime` property) to restore the previous behavior of forcing the build runtime for all IDEs JetBrains/intellij-platform-gradle-plugin#1611

### Removed

- Removed the deprecated `setupDependencies` task (`SetupDependenciesTask`), a legacy leftover from the `1.x`→`2.x` migration. The IntelliJ Platform dependency is now declared directly in Gradle dependencies, so the task is no longer needed. Remove any references to `setupDependencies` (including "After Sync" Gradle triggers). This is a breaking change and updates the plugin's public API.

### Fixed

- Respect `kotlin.stdlib.default.dependency` values declared in subproject `gradle.properties` files when checking for a Kotlin stdlib dependency conflict JetBrains/intellij-platform-gradle-plugin#1789
- Honor an explicitly configured Java version when computing the `org.gradle.jvm.version` attribute of the published `intellijPlatformComposedJar` and `intellijPlatformDistribution` variants (and the corresponding compilation). The target IntelliJ Platform Java version is now applied only through the Java toolchain, so Gradle natively derives `sourceCompatibility`/`targetCompatibility` and every supported way of configuring the Java level is respected — `java { sourceCompatibility / targetCompatibility }`, `java { toolchain { languageVersion } }`, `tasks.compileJava { options.release }`, and `tasks.withType<JavaCompile> { targetCompatibility }`. As part of this change, the plugin no longer sets an implicit `options.release` convention on `JavaCompile` tasks; `compileJava.options.release` is now `null` unless configured explicitly (the default bytecode target is unchanged and still follows the target platform via the toolchain) JetBrains/intellij-platform-gradle-plugin#1772
- Fail the `verifyPlugin` task based on the IntelliJ Plugin Verifier report files instead of parsing its console output, so verification problems matching the configured `failureLevel` fail the build even when `teamCityOutputFormat` is enabled. The task now fails closed when the verifier produces no verdict (e.g. an unparseable plugin artifact, detected from both plain and TeamCity output), and rejects combining the `NOT_DYNAMIC` failure level with `teamCityOutputFormat` (including via `FailureLevel.ALL`, which contains `NOT_DYNAMIC`) since that status is neither persisted nor emitted as a TeamCity message JetBrains/intellij-platform-gradle-plugin#1739
- Keep test dependencies that differ only by a classifier on the classpath. Since `2.2.0` the plugin forced the `composed-jar` library-elements attribute on the whole compile, test-compile, and test-runtime classpaths, so an external module pulled both plain and through a transitive `:tests` classifier collapsed to a single `composed-jar` node and its classifier artifact was dropped, which broke `compileTestJava`/`compileTestKotlin`. The attribute is now requested per project dependency, so the project's own instrumented and composed modules still resolve through `composed-jar` while third-party modules use standard Gradle variant selection JetBrains/intellij-platform-gradle-plugin#1889
...
- Resolve the `testFramework(...)` dependency declared inside a custom `intellijPlatformTesting.testIde` block against that task's own target IntelliJ Platform build number instead of the base project platform, so a custom task targeting a different `type`, `version`, or `localPath` no longer resolves the test framework for the wrong build JetBrains/intellij-platform-gradle-plugin#1993
...
- Attach plugin API sources bundled in a plugin distribution's `lib/src` directory. Dependencies declared with `bundledPlugin(...)` or `localPlugin(...)` now publish `lib/src/*.jar` files through Ivy's `sources` configuration, allowing the IDE to attach them automatically without adding them to compile or runtime classpaths. Transitive bundled plugin/module dependencies are mapped explicitly to the Ivy `default` configuration so the added `sources` configuration does not duplicate them in the resolved dependency graph. Local plugin archives are extracted into a content-fingerprinted directory only once: an already-materialized directory is reused instead of being re-extracted, avoiding redundant work and, on Windows, extraction hangs caused by overwriting JARs that a previously-resolved classpath still keeps locked JetBrains/intellij-platform-gradle-plugin#2033
...
- Avoid resolving `intellijPlatformDependency` while Gradle tooling models query Java and Kotlin compile target conventions during IDE sync. JetBrains/intellij-platform-gradle-plugin#2157
