...
- Make archive extraction idempotent: `ExtractorService.extract()` now reuses an already-materialized, non-empty target directory instead of re-extracting the archive. The reuse check runs inside the extraction `BuildService`, so it is not recorded as a configuration-cache input; besides avoiding wasted work when a single extracted IDE is shared across builds/workers, it prevents Windows `AccessDenied` failures caused by overwriting JARs still locked by a previously-resolved classpath
- Reuse the already-extracted base IntelliJ Platform for the IntelliJ Plugin Verifier when a requested verification IDE matches the base platform by type, version, and product mode, instead of downloading and extracting a second, identical copy of the IDE for `verifyPlugin`. IDEs that differ from the base platform are still resolved and extracted as before
