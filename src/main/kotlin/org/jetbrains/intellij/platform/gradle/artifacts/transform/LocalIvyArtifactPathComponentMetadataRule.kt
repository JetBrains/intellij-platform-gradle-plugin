 ...
 *
 *  For bundledPlugin & bundledModule, we expect:
 *  - "artifact" (IvyModule#name) is mandatory and contains only the name of the artifact (e.g. jar archive),
 *    without the extension.
 *
 *  - "url" (IvyModule#type) contains a path, relative to the platformPath (IDE), without the file's name.
 ...
 *
 *    The reason why we put the path into "url" is that the name should not have it, because:
 ...
 *
 * Relative paths are better than absolute because if Gradle's dependency verification is used with metadata
 * (e.g., ivy.xml or pom.xml) files verification enabled, hashes of these files will be the same in different
 * environments, despite that they are stored in different locations. If absolute paths are used, they will be
 * mentioned in ivy.xml thus changing the hash on each env.
 *
 * But since our local Ivy repository has an artifact pattern "/[artifact]" relative URLs won't work.
 * See [IntelliJPlatformRepositoriesHelper.createLocalIvyRepository]. That is why this class is needed.
 *
...
abstract class LocalIvyArtifactPathComponentMetadataRule @Inject constructor(
    ...
    override fun execute(context: ComponentMetadataContext) {
        ...
        context.details.allVariants {
            withFiles {
                // Unfortunately, Gradle here does not expose anything from Ivy metadata, all we know is:
                // group, name and version. Much more is visible in debug, but all that is private.
                ...
                ivyModule.publications.forEach { artifact ->
                    ...
                    // It is important to pass in the name & abs path as the second arg, instead of just
                    // addFile(absPathString), because when only the path is given Gradle is thinking that it is
                    // downloading a file from a URL and copied all artifacts into:
                    // ~/.gradle/caches/modules-2/files-2.1/

                    if (OperatingSystem.current().isWindows) {
                        // On Windows we should add a leading slash because there absolute paths start from a drive
                        // letter, but if Gradle sees such path (without a leading slash) it will treat it relative
                        // to the build dir and absPathString will become malformed like:
